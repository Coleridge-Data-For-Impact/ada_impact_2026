...
DROP TABLE IF EXISTS tr_state_impact_ada_training.FACT_Person_Program_Participation;


INSERT INTO tr_state_impact_ada_training.FACT_Person_Program_Participation (Person_Key, Intake_Characteristics_Key, Program_Key, Entry_Year_Quarter_Key, Exit_Year_Quarter_Key,
...
CREATE TABLE tr_state_impact_ada_training.FACT_Person_Program_Participation AS                                               
...
(
    ...
    FROM ds_ar_dws.pirl_update pirl_update
    LEFT JOIN tr_state_impact_ada_training.DIM_State states
        ON pirl_update.state_code_of_residence_wioa = states.state_abbreviation
    WHERE DATEPART(year, date_of_program_entry_wioa) >= 2010 and pirl_update.sheetnameproperty  = 'Wagner-Peyser'
...
cteFactData (Person_Key, Intake_Characteristics_Key, Program_Key, Entry_Year_Quarter_Key, Exit_Year_Quarter_Key, County_of_Residence_Key,
             ...
             Enrolled_in_Secondary_Education_Program, Date_Enrolled_in_Post_Exit_Education_or_Training_Program,  Incarcerated_at_Program_Entry, Date_Released_from_Incarceration, Other_Reason_for_Exit,
...
(
    SELECT  --Lookup Person Surrogate Key
            ...
            (
                SELECT Intake_Characteristics_Key
                FROM tr_state_impact_ada_training.DIM_Intake_Characteristics
            ...
            (
                ...
                FROM tr_state_impact_ada_training.DIM_Year_Quarter entry_qtr
            ...
            (
                ...
                FROM tr_state_impact_ada_training.DIM_Year_Quarter exit_qtr
            ...
            (
                ...
                FROM tr_state_impact_ada_training.DIM_County
            ...
            (
                ...
                FROM tr_state_impact_ada_training.DIM_State
            ...
            (
                ...
                FROM tr_state_impact_ada_training.DIM_CIP
...
--Person_Key, Intake_Characteristics_Key, Program_Key, Entry_Year_Quarter_Key, Exit_Year_Quarter_Key,
---                                               County_of_Residence_Key, State_of_Residence_Key, CIP_Classification_Key,
SELECT  COALESCE(Person_Key, 0) as person_key,
        COALESCE(Intake_Characteristics_Key, 0) as intake_characteristics_key,
        COALESCE(Program_Key, 0) as program_key,
        COALESCE(Entry_Year_Quarter_Key, 0) as entry_year_quarter_key,
        COALESCE(Exit_Year_Quarter_Key, 0) as exit_year_quarter_key,
        COALESCE(County_of_Residence_Key, 0) as county_of_residence_key,
        COALESCE(State_of_Residence_Key, 0) as state_of_residence_key,
        COALESCE(CIP_Classification_Key, 0) as cip_classification_key,
